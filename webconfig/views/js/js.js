$(document).ready(function(){
    ...
    var webconfig_grid = $("#grid-webconfig").bootgrid({
        ...
        multiSelect: true,
        templates: {
            header:"<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\"><div class=\"{{css.search}}\"></div></div><div class=\"col-sm-6\"><div class=\"{{css.actions}}\"></div> <div class='btn btn-primary' id='create' class='command-add'> <span class=\"fa fa-plus-square-o\"></span> New Webconfig</div></div></div></div>",
        },
        formatters: {
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $(this).find(".command-edit").on("click", function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    $("#grid-webconfig").bootgrid("reload");
                    setTimeout(function(){
                        $('#myModal').modal('hide')
                    }, 10000);
                }
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    webconfig_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#mywebconfig').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myForm(status,e) {
        ...
        if(status == 'edit') {
            ...
            $('#mywebconfig .modal-title').html('Edit webconfig '+e.data("row-id"));
	 ...
	 $('#content').val(data.content);
            ...
            $('#mywebconfig .modal-title').html('Create New webconfig ');
            
        }

        $('#mywebconfig').modal('show');
    ...
    function myAlert(e)
    {
        ...
        mesg["alert"] = e.alert;
        mesg["title"] = e.msg;
        mesg["msg"] = "#webconfig "+e._id+" "+e.msg;
        notif_show(mesg);
    }
